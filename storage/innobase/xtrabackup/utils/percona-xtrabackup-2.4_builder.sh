...
get_sources(){
    ...
    #
    sed -i 's:13:14:' XB_VERSION 
    source XB_VERSION
    cat XB_VERSION > ../percona-xtrabackup-2.4.properties
    echo "REVISION=${REVISION}" >> ../percona-xtrabackup-2.4.properties
    BRANCH_NAME="${BRANCH}"
    echo "BRANCH_NAME=$(echo ${BRANCH_NAME} | awk -F '/' '{print $(NF)}')" >> ../percona-xtrabackup-2.4.properties
    echo "PRODUCT=Percona-XtraBackup-2.4" >> ../percona-xtrabackup-2.4.properties
    echo "PRODUCT_FULL=Percona-XtraBackup-${XB_VERSION_MAJOR}.${XB_VERSION_MINOR}.${XB_VERSION_PATCH}${XB_VERSION_EXTRA}" >> ../percona-xtrabackup-2.4.properties
    echo "PRODUCT_UL_DIR=Percona-XtraBackup-2.4" >> ../percona-xtrabackup-2.4.properties
    PRODUCT="Percona-XtraBackup-2.4"
    ...
    echo "DESTINATION=${DESTINATION}" >> percona-xtrabackup-2.4.properties

    enable_venv

    $CMAKE_BIN . -DDOWNLOAD_BOOST=1 -DWITH_BOOST=${WORKDIR}/boost -DWITH_MAN_PAGES=1
    make dist
    #
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH}/${REVISION}/${BUILD_ID}" >> ../percona-xtrabackup-2.4.properties
    rm -rf ${PXBDIR}
...
#                rm procps-ng-devel-3.3.15-6.el8.x86_64.rpm
          ...
          #  PKGLIST+=" procps-ng-devel"
            if [[ "${RHEL}" -eq 7 ]]; then
                PKGLIST+=" numactl-libs libopcodes perl-Digest-MD5  python3-pip python3-setuptools python3-wheel rh-python36-python-sphinx"
            elif [[ "${RHEL}" -eq 6 ]]; then
...
build_srpm(){
    ...
    VERSION=$(echo ${TARFILE}| awk -F '-' '{print $3}'|awk -F '.' '{print $1"."$2"."$3}')
    #
...
build_rpm(){
    ...
    #
    echo "RHEL=${RHEL}" >> ${CURDIR}/percona-xtrabackup-2.4.properties
    echo "ARCH=${ARCH}" >> ${CURDIR}/percona-xtrabackup-2.4.properties
    #
...
build_source_deb(){
    ...
    echo "DEB_RELEASE=${DEB_RELEASE}" >> ${CURDIR}/percona-xtrabackup-2.4.properties

    NEWTAR=${NAME}-24_${VERSION}.orig.tar.gz
    mv ${TARFILE} ${NEWTAR}
...
build_deb(){
    ...
    #
    echo "DEB_RELEASE=${DEB_RELEASE}" >> ${CURDIR}/percona-xtrabackup-2.4.properties
    echo "DEBIAN_VERSION=${OS_NAME}" >> ${CURDIR}/percona-xtrabackup-2.4.properties
    echo "ARCH=${ARCH}" >> ${CURDIR}/percona-xtrabackup-2.4.properties
